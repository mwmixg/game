...
class Game {
  ...
  _updateObjects(dt) {
    ...
    for (const obj of this.objects) {
      ...
      /* — Sprite interaction: star attraction to player — */
      if ((obj.type === 'star' || obj.type === 'bonus' || obj.type === 'superstar') && obj.y > 0) {
        const dx = playerCenterX - obj.x;
        if (Math.abs(dx) < 220) {
          const force = 0.35 * (1 - Math.abs(dx) / 220) * dt * 60;
          obj.x += Math.sign(dx) * force;
          obj.x = Math.max(obj.radius, Math.min(this.canvas.width - obj.radius, obj.x));
        }
      }
    ...
    /* — Sprite interaction: meteor repulsion of stars — */
    this.interactionFx = [];
    const stars = remaining.filter(o => o.type === 'star');
    const meteors = remaining.filter(o => o.type === 'meteor');
    for (const meteor of meteors) {
      for (const star of stars) {
        const dx = star.x - meteor.x;
        const dy = star.y - meteor.y;
        const dist = Math.sqrt(dx * dx + dy * dy);
        if (dist < 80 && dist > 0) {
          const pushForce = 1.2 * (1 - dist / 80) * dt * 60;
          const nx = dx / dist, ny = dy / dist;
          star.x += nx * pushForce;
          star.y += ny * pushForce;
          star.x = Math.max(star.radius, Math.min(this.canvas.width - star.radius, star.x));
          this.interactionFx.push({
            type: 'repulse',
            x1: meteor.x, y1: meteor.y,
            x2: star.x, y2: star.y,
            life: 0.3
          });
        }
  ...
  _handleCatch(obj) {
    ...
    /* — Track combo chain positions — */
    if (obj.type !== 'meteor') {
      this.caughtChain.push({ x: cx, y: cy });
      if (this.caughtChain.length > 5) this.caughtChain.shift();
    }
  ...
  _render() {
    ...
    this.renderer.drawSpriteFx(this.objects, this.player, this.caughtChain, this.combo, this.interactionFx);
