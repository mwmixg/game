class Renderer {
  ...
  drawSpriteFx(objects, player, caughtChain, combo, interactionFx) {
    const ctx = this.ctx;
    const playerCenterX = player.x + player.width / 2;
    const playerCenterY = player.y + player.height / 2;

    /* — 1. Star attraction glow — */
    ...
    /* — 2. Meteor repulsion flash — */
    for (const fx of interactionFx) {
      if (fx.type === 'repulse') {
        const alpha = fx.life * 0.6;
        ctx.beginPath();
        ...
        ctx.shadowBlur = 0;
        ctx.beginPath();
        ctx.arc(fx.x2, fx.y2, 4 * fx.life, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(231, 76, 60, ' + alpha * 0.4 + ')';
        ctx.fill();
      }
    }

    /* — 3. Combo chain sparkles — */
    if (combo >= 3 && caughtChain.length >= 2) {
      const chain = caughtChain;
      for (let i = 0; i < chain.length - 1; i++) {
        const a = chain[i], b = chain[i + 1];
        const alpha = 0.15 + 0.1 * Math.sin(performance.now() / 200 + i);
        ctx.beginPath();
        ctx.moveTo(a.x, a.y);
        ctx.lineTo(b.x, b.y);
        ctx.strokeStyle = 'rgba(255, 215, 0, ' + alpha + ')';
